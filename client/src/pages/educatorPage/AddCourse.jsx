...
const AddCourse = () => {
  ...
  return (
    <div className="min-h-screen overflow-scroll flex flex-col items-start md:p-8 md:pb-0 p-4 pt-8 pb-0">
      <form onSubmit={handleSubmit}
        ...
        <div className="flex items-start justify-between gap-8">
          <div className="flex flex-col gap-1">
            ...
            <input onChange={e => setCoursePrice(e.target.value)} value={coursePrice}
              type="number" placeholder="0" className="outline-none md:py-2.5 py-2 px-3 w-28 rounded border border-gray-500" />
          </div>
          <div className="flex flex-col gap-1">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input onChange={e => setDiscount(e.target.value)} value={discount}
            type="number" placeholder="0" min={0} max={100} className="outline-none md:py-2.5 py-2 px-3 w-28 rounded border border-gray-500" />
        </div>
        {/*Adding Chapters & Lectures */}
        <div>
          {chapters.map((chapter, chaperIndex) => (
            <div key={chaperIndex} className="bg-white border rounded-lg mb-4">
              <div className="flex justify-between items-center p-4 border-b">
                <div className="flex items-center">
                  <img onClick={() => handleChapter('toggle', chapter.chapterId)}
                    className={`mr-2 cursor-pointer transition-all duration-200 ${chapter.collapsed && '-rotate-90'}`}
                    src={assets.dropdown_icon} width={14} alt="" />
                  <span className="font-semibold" > {chaperIndex + 1} {chapter.chapterTitle} </span>
                </div>
