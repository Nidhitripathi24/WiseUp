  ...
  return (
    <>
      <div className='md:px-36 px-8 pt-10'>
        ...
        <table className='md:table-auto table-fixed w-full overflow-hidden border border-gray-700 mt-10'>
          <thead className='text-white border-b border-gray-700  text:sm text-left max-sm:hidden bg-gray-900'>
            <tr>
          ...
          <tbody className='text-gray-300'>
            {enrolledCourses.map((course, index) => (
              <tr className='border-b border-gray-700 hover:bg-gray-900/50 transition-colors'
                key={index}>
                <td className='md:px-4 pl-2 md:pl-4 py-3 flex items-center space-x-3'>
                  <img src={course.courseThumbnail} alt='' className='w-14 sm:w-24 md:w-28' />
                  <div className='flex-1 '>
                    <p className='mb-1 max-sm:text-sm text-white'>
                      {course.courseTitle}
                    </p>
                ...
                <td className='px-4 py-3 max-sm:idden'>
                  {progressArray[index] && `${progressArray[index].lectureCompleted}/${progressArray[index].totalLectures}`} <span>Lectures</span>
                </td>
                <td className='px-4 py-3 max-sm:text-right'>
                  <button onClick={() => navigate('/player/' + course._id)} className='px-3 sm:px-5 py-1.5 sm:py-2 bg-blue-600 hover:bg-blue-700 transition-colors max-sm:text-xs text-white rounded'>
